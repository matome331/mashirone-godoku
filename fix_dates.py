...
# Fix mixed encoding issue in video_dates_clean.json
with open(dates_path, 'r', encoding='utf-8') as f:
    bad_dates = json.load(f)

video_dates = {}
for k, v in bad_dates.items():
    clean_k = k.replace('\x00', '').strip()
    clean_v = v.replace('\x00', '').strip()
    # Format YYYYMMDD to YYYY/MM/DD if missed
    match = re.search(r'^(\d{4})(\d{2})(\d{2})$', clean_v)
    if match:
        clean_v = f"{match.group(1)}/{match.group(2)}/{match.group(3)}"
    video_dates[clean_k] = clean_v

# overwrite cleaned json
with open(dates_path, 'w', encoding='utf-8') as f:
    json.dump(video_dates, f)

# Reload comments and regenerate refined misreadings
...
for item in data:
    ...
    if v_date == "NA" or v_date == "": v_date = "日付不明"
...
with open(output_file, 'w', encoding='utf-8') as f:
    f.write("真白猫ミミィ 漢字・英単語誤読まとめ（厳選版 v15 全配信日完全対応）\n")
    f.write("="*60 + "\n\n")
    current_video = ""
...
print(f"抽出数: {len(refined_list)} 件, うち日付不明: {unknowns}件")
